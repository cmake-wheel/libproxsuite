...
consume(const int*, int) noexcept
{
  return 0;
}
}
...
DOCTEST_INTERFACE String
toString(const Contains& in);

DOCTEST_INTERFACE bool
operator==(const String& lhs, const Contains& rhs);
DOCTEST_INTERFACE bool
operator==(const Contains& lhs, const String& rhs);
DOCTEST_INTERFACE bool
operator!=(const String& lhs, const Contains& rhs);
DOCTEST_INTERFACE bool
operator!=(const Contains& lhs, const String& rhs);
...
DOCTEST_INTERFACE const char*
assertString(assertType::Enum at);
DOCTEST_INTERFACE const char*
...
struct ContextOptions //! OCLINT too many fields
{
  ...
  bool
    force_colors; // forces the use of colors even when a tty cannot be detected
  bool no_breaks; // to not break into the debugger
  bool no_skip;   // don't skip test cases which are marked to be skipped
  bool
    gnu_file_line; // if line numbers should be surrounded with :x: and not (x):
...
toString()
{
  ...
  String ret =
    __FUNCSIG__; // class doctest::String __cdecl doctest::toString<TYPE>(void)
...
template<typename T,
         typename detail::types::enable_if<!detail::types::is_enum<T>::value,
                                           bool>::type = true>
String
...
toString(char in);
DOCTEST_INTERFACE String
toString(char signed in);
DOCTEST_INTERFACE String
toString(char unsigned in);
DOCTEST_INTERFACE String
toString(short in);
DOCTEST_INTERFACE String
toString(short unsigned in);
DOCTEST_INTERFACE String
toString(signed in);
DOCTEST_INTERFACE String
toString(unsigned in);
DOCTEST_INTERFACE String
toString(long in);
DOCTEST_INTERFACE String
toString(long unsigned in);
DOCTEST_INTERFACE String
toString(long long in);
DOCTEST_INTERFACE String
toString(long long unsigned in);
...
DOCTEST_INTERFACE const ContextOptions*
getContextOptions();
...
#endif
DOCTEST_INTERFACE String
toString(IsNaN<float> in);
DOCTEST_INTERFACE String
toString(IsNaN<double> in);
DOCTEST_INTERFACE String
...
String
stringifyBinaryExpr(const DOCTEST_REF_WRAP(L) lhs,
                    const char* op,
...
DOCTEST_INTERFACE bool
decomp_assert(assertType::Enum at,
              const char* file,
              int line,
              const char* expr,
              const Result& result);
...
binary_assert(assertType::Enum at,
              const char* file,
              int line,
              const char* expr,
...
template<typename L>
DOCTEST_NOINLINE bool
unary_assert(assertType::Enum at,
             const char* file,
             int line,
             const char* expr,
...
DOCTEST_INTERFACE doctest::detail::TestSuite&
getCurrentTestSuite();
} // namespace doctest_detail_test_suite_ns
...
DOCTEST_INTERFACE void
registerReporterImpl(const char* name,
                     int prio,
...
#define DOCTEST_TEST_CASE_FIXTURE(c, decorators)                               \
  DOCTEST_IMPLEMENT_FIXTURE(DOCTEST_ANONYMOUS(DOCTEST_ANON_CLASS_),            \
                            c,                                                 \
                            DOCTEST_ANONYMOUS(DOCTEST_ANON_FUNC_),             \
                            decorators)
...
void
String::setOnHeap() noexcept
{
  *reinterpret_cast<unsigned char*>(&buf[last]) = 128;
}
void
String::setLast(size_type in) noexcept
{
  buf[last] = char(in);
}
void
String::setSize(size_type sz) noexcept
{
  if (isOnStack()) {
    buf[sz] = '\0';
    setLast(last - sz);
...
char
String::operator[](size_type i) const
{
...
String
String::substr(size_type pos, size_type cnt) &&
{
...
String
String::substr(size_type pos, size_type cnt) const&
{
  cnt = std::min(cnt, size() - 1 - pos);
...
String::size_type
String::find(char ch, size_type pos) const
{
...
String::size_type
String::rfind(char ch, size_type pos) const
{
  const char* begin = c_str();
...
bool
Contains::checkWith(const String& other) const
{
...
namespace detail {
void
filldata<const void*>::fill(std::ostream* stream, const void* in)
{
  if (in) {
    *stream << in;
  } else {
...
unsigned long long
hash(const SubcaseSignature& sig)
{
  return hash(hash(hash(sig.m_file), hash(sig.m_name.c_str())), sig.m_line);
}

unsigned long long
hash(const std::vector<SubcaseSignature>& sigs, size_t count)
{
...
unsigned long long
hash(const std::vector<SubcaseSignature>& sigs)
{
  unsigned long long running = 0;
...
AssertData::AssertData(assertType::Enum at,
                       const char* file,
                       int line,
                       const char* expr,
                       const char* exception_type,
...
struct ConsoleReporter : public IReporter
{
  ...
  void successOrFailColoredStringToStream(bool success,
                                          assertType::Enum at,
  ...
  void test_run_end(const TestRunStats& p) override
  {
    ...
    auto totwidth =
      int(std::ceil(log10((std::max(p.numTestCasesPassingFilters,
                                    static_cast<unsigned>(p.numAsserts))) +
                          1)));
    auto passwidth = int(std::ceil(log10(
      (std::max(p.numTestCasesPassingFilters - p.numTestCasesFailed,
                static_cast<unsigned>(p.numAsserts - p.numAssertsFailed))) +
      1)));
    auto failwidth = int(
      std::ceil(log10((std::max(p.numTestCasesFailed,
